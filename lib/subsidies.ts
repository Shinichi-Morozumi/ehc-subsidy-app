...
const RAW_SUBSIDIES: Subsidy[] = [
  {
    ...
    /* 2026-09-16 EHC-0039 台帳#32 で追加。
       この制度は**令和7年度補正予算**の事業（＝2025年度）で、
       3次公募の受付だけが 2026-08-20〜09-28（2026年度）に行われる。
       対象製品一覧・公募要領は令和7年度補正のものを見るので、
       確認記録の年度キーも 2025 でなければ実態とずれる。
       宣言前は fiscalYearOf(todayJst()) が 2026 を返していた（lib/targetProduct.ts の
       targetProductKeyOf のコメント参照）。 */
    programFiscalYear: 2025,
  ...
  },
  {
    ...
    applyClose: "2026-09-28",
    scheduleNote: "3次公募 受付中（2026/8/20〜9/28 17:00必着）。交付決定は11月下旬予定。1次・2次は終了",
    // 2026-09-16 台帳#32。sii_iv と同一公募＝令和7年度補正（2025年度）の事業。
    programFiscalYear: 2025,
    /* 2026-09-16 EHC-0039 台帳#32 で追加。
       この制度は枠が2つあり、補助率が違う（メーカー強化枠 1/3以内 ／
       トップ性能枠 更新1/2以内・新設1/5以内）。対象製品一覧も枠ごとに別物。
       宣言前は applicationFrame 未宣言のため NO_FRAME_LABEL という1枠に潰れており、
       どちらか一方の枠で取った確認記録が制度全体の確認済みへ繰り上がる状態だった。
       枠が2件ある間、対象製品の照合は frame_undecided で止まる（checked は false）。
       名前は公募要領の表記に合わせる。ここを書き換えると、旧ラベルで取った
       確認記録はキー不一致で自動的に効かなくなる。それでよい。 */
    applicationFrames: ["メーカー強化枠", "トップ性能枠"],
