...
/* ───────────────────────────────────────────────────────────
   ...
   理由:
   ・【2026-09-10 EHC-0038 P0-6 で是正済み】lib/pricing.ts の clampDropinRate は
     下限0.1のため、実測が「消費増（負値）」でも画面には「10%削減」と出ていた。
     大塚倉庫の2025→2026 1〜8月は +2.6%（消費増）で、まさにこの状態だった。
     下限を撤去し、未確認=null／0%=0／負値はそのまま、へ改めた。
   ・DROPIN_REFRI_RATE の unknown 既定 0.25 は【未是正】。未確認を数値化してしまう。
   ・呼び出し側（DropinRoiWizard / DropinSimulator）は null を 0 に落として
     「計算に織り込まない」扱いにしているだけで、まだ「未確認」として
     画面に出す表示は持っていない。

   コードは削除しない。フラグを true に戻せば復帰できる状態で残す。
   復帰の条件は EHC-0033 §3 の符号規約（未確認=null／0%=0／増加=負値）への
   是正と、施工日の確定。
   → 符号規約のうち clampDropinRate 側は済み。残るのは
     DROPIN_REFRI_RATE の unknown 既定の是正・未確認表示のUI・施工日の確定。
   ─────────────────────────────────────────────────────────── */
