  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {matched.map((a) => (
            <div key={a.id} className="bg-night-900 border border-white/10 rounded-lg p-3">
              ...
              <div className="text-xs text-slate-500 mb-2">{a.equipment} {a.units}台</div>
      ...
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
        ...
        <div className="text-xs text-slate-400 mt-2">公共施設での実証データ（{AKITA_RICE_WAREHOUSE.comparisonPeriod}）</div>
      </div>
      ...
      <div className="bg-ehc-500/10 border border-ehc-500/30 rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
          <div className="bg-night-900 border border-ehc-500/20 rounded-md p-2">
            <div className="text-xs text-slate-500">電力削減（同温度補正）</div>
            <div className="text-base font-bold text-ehc-300">−{EHC_FIELD_TEST_2026.reductionRate}%</div>
          </div>
          {/* 2026-08-24 監査: 年間削減額・年間CO2は annualKwh と単価・CO2係数で再計算しても
              一致しない（lib/achievements.ts のコメント参照）。整合が取れるまで数値を断定表示しない。 */}
          <div className="bg-night-900 border border-ehc-500/20 rounded-md p-2">
            <div className="text-xs text-slate-500">年間削減コスト</div>
            {EHC_FIELD_TEST_2026.verifiedAnnualFigures ? (
              ...
              <div className="text-sm font-bold text-slate-400">検証中</div>
            )}
          </div>
          <div className="bg-night-900 border border-ehc-500/20 rounded-md p-2">
            <div className="text-xs text-slate-500">年間CO2削減</div>
            {EHC_FIELD_TEST_2026.verifiedAnnualFigures ? (
          ...
          <div className="bg-night-900 border border-ehc-500/20 rounded-md p-2">
            <div className="text-xs text-slate-500">予測安定性</div>
